...
a = np.pi / 180.0 * -19.3
pXY = pts[:,0:2] * np.mat([[np.cos(a), np.sin(a)], [-np.sin(a), np.cos(a)]]) + [-4, 0]
pXY = pXY
pZ = np.mat(pts[:,2]).T
...
ar.differenceAndDelete((houseWidth - owt, 5.5, 1.2), (houseWidth, 5.5+winW, 1.0+winH), "window4", "ground_w4", extendAlongAxis = 0)  # window 4
...
ar.wall(r1_x, r1_y - iwt*1.5, houseWidth, r1_y, "frontWallRoom1_b")
...
ar.differenceAndDelete((0, 6.4, 1.0), (owt, 6.4+winW, 1.0+winH), "window14", "first_w3", extendAlongAxis = 0)  # window 14: from kitchen to garden
...
ar.differenceAndDelete((houseWidth - owt, 6.5, 1.0), (houseWidth, 6.5+winW, 1.0+winH), "window18", "first_w4", extendAlongAxis = 0)  # window 18
...
ar.wallStartSize(leftRoom6, y, iwt, h, "wallLeftRoom6_a")
ar.wallStartSize(leftRoom5, y, iwt, h, "wallLeftRoom6_b")
